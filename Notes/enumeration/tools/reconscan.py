...
def write_to_file(ip_address, enum_type, data):
    ...
    file_path_linux = '%s.md' % (ip_address)
    file_path_windows = '%s.md' % (ip_address)
...
def dirb(ip_address, port, url_start):
    ...
    DIRBSCAN = "dirb %s:%s:%s -o %sdirb-%s.txt -r" % (url_start, ip_address, port, ip_address, ip_address)
...
def nikto(ip_address, port, url_start):
    ...
    NIKTOSCAN = "nikto -h %s:%s -o %snikto-%s-%s.txt" % (url_start, ip_address, ip_address, url_start, ip_address)
...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt,http-devframework,http-enum,http-frontpage-login,http-git,http-iis-webdav-vuln,http-php-version,http-robots.txt,http-shellshock,http-vuln-cve2015-1635 -oN %s%s_http.nmap %s" % (port, ip_address, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    SSLSCAN = "sslscan %s:%s >> %sssl_scan_%s" % (ip_address, port, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oN %smssql_%s.nmap %s" % (port, ip_address, ip_address)
...
def smtpEnum(ip_address, port):
    ...
    SMTPSCAN = "nmap -sV -Pn -p %s --script=smtp-commands,smtp-enum-users,smtp-vuln-cve2010-4344,smtp-vuln-cve2011-1720,smtp-vuln-cve2011-1764 %s -oN %ssmtp_%s.nmap" % (port, ip_address, ip_address, ip_address)
...
def smbEnum(ip_address, port):
    print "INFO: Detected SMB on " + ip_address + ":" + port
    enum4linux = "enum4linux -a %s > %senum4linux_%s" % (ip_address, ip_address, ip_address)
...
def ftpEnum(ip_address, port):
    ...
    FTPSCAN = "nmap -sV -Pn -vv -p %s --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221 -oN '%sftp_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
...
def udpScan(ip_address):
    ...
    UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '%sudp_%s.nmap' %s"  % (ip_address, ip_address, ip_address)
    ...
    UNICORNSCAN = "unicornscan -mU -v -I %s > %sunicorn_udp_%s.txt" % (ip_address, ip_address, ip_address)
...
def nmapScan(ip_address):
    ...
    print bcolors.OKGREEN + "INFO: Running general TCPUDP nmap scans for " + ip_address + bcolors.ENDC


    TCPSCAN = "nmap -sV -O %s -oN '%s%s.nmap'"  % (ip_address, ip_address, ip_address)
    ...
    for line in lines:
        ...
        if ("tcp" in line) and ("open" in line) and not ("Discovered" in line):
            ...
            port = line.split(" ")[0] # grab the portproto
            # print port
   ...
   # go through the service dictionary to call additional targeted enumeration functions
    for serv in serv_dict:
        ...
        elif (serv == "sslhttp") or ("https" == serv) or ("https?" == serv):
            for port in ports:
                port = port.split("")[0]
...
if __name__=='__main__':
    ...
    for scanip in targets:
        ...
        if not scanip in dirs:
            ...
            subprocess.check_output("mkdir h " + scanip, shell=True)
            subprocess.check_output("mkdir h " + scanip + "exploits", shell=True)
            subprocess.check_output("mkdir h " + scanip + "privesc", shell=True)
            print bcolors.OKGREEN + "INFO: Folder created here: " + "" + scanip + bcolors.ENDC
            subprocess.check_output("cp rootoscpreportswindows-template.md " + scanip + "mapping-windows.md", shell=True)
            subprocess.check_output("cp rootoscpreportslinux-template.md " + scanip + "mapping-linux.md", shell=True)
            print bcolors.OKGREEN + "INFO: Added pentesting templates: " + "" + scanip + bcolors.ENDC
            subprocess.check_output("sed -i -e 'sINSERTIPADDRESS" + scanip + "g' " + scanip + "mapping-windows.md", shell=True)
            subprocess.check_output("sed -i -e 'sINSERTIPADDRESS" + scanip + "g' " + scanip + "mapping-linux.md", shell=True)
